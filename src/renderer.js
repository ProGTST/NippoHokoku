...
let pendingLoginEmail = ''; // 第2フェーズ手動入力時、永続化に添える今回ログインのメール（無ければ空）
...
// 返り値: 成功時 { code, name }／失敗時 { error: 'reauth' } または
//         { error: 'manual', email }（第2フェーズのユーザーID手動入力へ誘導）。
async function resolveLoginTanto() {
  ...
  if (email) {
    // 第1フェーズ: メールのローカル部（@ より前）＝名称4 で照合
    const hit = await lookupTantoByLocal(String(email).split('@')[0]);
    if (hit) {
    ...
    if (saved && saved.code && sameEmail(saved.email, email)) {
      return { code: saved.code, name: saved.name || '' };
    }
    // 第1フェーズ失敗 → 第2フェーズ（ユーザーID手動入力）。メールは永続化のため添える。
    return { error: 'manual', email };
  }
  // 2) サイレントSSO: 永続化した本人情報を再利用（手動確定分もここで自動認証される）
  const saved = await getSavedIdentity();
  ...
  // 4) 再認証後もメールを捕捉できず、永続情報も無い → 第2フェーズ（手動入力）
  return { error: 'manual', email: '' };
}

// 永続化した本人情報 { email, code, name } を安全に読む（無ければ null）。
...
// 2つのメールアドレスが（前後空白・大文字小文字を無視して）同一か。
...
// 第2フェーズも失敗した（手動入力したユーザーIDも名称4 に該当なし）時の最終エラー文言。
const NO_TANTO_MSG =
  '担当者情報が存在しないため、ログインできません。<br>上長か、管理者にお問い合わせください。';

// ---- 第2フェーズ: ユーザーID手動入力 --------------------------------------
// 第1フェーズ（メール→名称4 照合）で本人を特定できなかった時に呼ぶ。
// email は捕捉できていれば入力欄の初期候補＆認証成功時の永続化に使う（無ければ空文字）。
function showManualAuth(email) {
  pendingLoginEmail = email || '';
  clearLoginError();
  const box = $('manualAuth');
  // 既に表示中なら（自動再試行で複数回呼ばれても）入力途中の値やフォーカスを乱さない。
  ...
  if (inp) {
    ...
    if (email && !inp.value) inp.value = String(email).split('@')[0];
    inp.focus();
    inp.select?.();
  }
}
...
function wire() {
  ...
  // 第2フェーズ: ユーザーID手動入力（ボタン／Enter どちらでも認証実行）
